...
```
Where you’d replace **my_github_token** with the access token generated earlier; this command stores the encrypted github token in the env: section inside the **.travis.yml** file exmple below:
```yml
env:
  global:
